...
class suratbedanamaController extends Controller
{
	...
	public function index($id_no)
	{
		...
		return view('formsurat/suratBedanama',['penduduk' => $penduduk,'id_noSurat' => $id_no]);
	}
	public function cetaksuratbedanama(Request $request,$id_noSurat)
	{
		...
		$datasurat = Suratbedanama::where('nomor_surat',$idinsertsurat)->get();

		// kalau sudah dipanggil semua data yang dibutuhkan tinggal di masukkan kedalam view suratnya
		$view = \View::make('surat/bedanama',['desa' =>$desa,'penduduk' => $penduduk, 'nosurat' =>$nosurat, 'tanggalsurat' => $tglSurat, 'datasurat' => $datasurat, 'kop' => $request['kop']]);

		$html_content = $view->render();

		PDF::SetTitle('Surat Bedanama PDF');
		PDF::AddPage();
		PDF::writeHTML($html_content, true, false, true, false, '');

		PDF::output('cetaksuratbedanama.pdf');
	}

	public function permohonan($namaSurat)
	{
		$bedanama = Suratbedanama::where('id_desa',auth()->user()->id_desa)->where('status_surat','belum diproses')->with('penduduk')->get();
		// sesuai dengan route
		$idsurat = 6;
		
		$formatnomorsurat = Nomorsurat::where('id_desa',auth()->user()->id_desa)->where('id_jenissurat','=',$idsurat)->first();
		$link = 'cetakPermohonanBedanama';
		return view ('admindesa/permohonan_surat',['namaSurat' => $namaSurat, 'data' => $bedanama,'link' => $link,'format_nomor_surat' => $formatnomorsurat->id_no]);
	...
	public function permohonanCetak(Request $request,$id,$noFormat)
	{
		...
		$datasurat = Suratbedanama::where('nomor_surat',$id)->get();
		
		$desa = Desa::where('id_desa',$datasurat[0]->id_desa)->with('kabupaten', 'kecamatan')->get();

		$updateSurat = Suratbedanama::where('nomor_surat',$id);
		...
		PDF::SetTitle('Surat Beda Nama PDF');
		PDF::AddPage();
		PDF::writeHTML($html_content, true, false, true, false, '');
