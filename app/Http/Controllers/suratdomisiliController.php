...
class suratdomisiliController extends Controller
{
	...
	public function index($id_no)
	{
		...
		return view('formsurat/suratDomisili',['penduduk' => $penduduk,'id_noSurat' => $id_no]);
	}
	public function cetaksuratdomisili(Request $request,$id_noSurat)
	{
		...
		$datasurat = Suratdomisili::where('nomor_surat',$idinsertsurat)->get();

		// kalau sudah dipanggil semua data yang dibutuhkan tinggal di masukkan kedalam view suratnya
		$view = \View::make('surat/domisili',['desa' =>$desa,'penduduk' => $penduduk, 'nosurat' =>$nosurat, 'tanggalsurat' => $tglSurat, 'datasurat' => $datasurat, 'kop' => $request['kop']]);

		$html_content = $view->render();

		PDF::SetTitle('Surat Domisili PDF');
		PDF::AddPage();
		PDF::writeHTML($html_content, true, false, true, false, '');

		PDF::output('cetaksuratdomisili.pdf');
	}

	public function permohonan($namaSurat)
	{
		$domisili = Suratdomisili::where('id_desa',auth()->user()->id_desa)->where('status_surat','belum diproses')->with('penduduk')->get();
		// sesuai dengan route
		$idsurat = 7;
		
		$formatnomorsurat = Nomorsurat::where('id_desa',auth()->user()->id_desa)->where('id_jenissurat','=',$idsurat)->first();

		$link = 'cetakPermohonanDomisili';
		return view ('admindesa/permohonan_surat',['namaSurat' => $namaSurat, 'data' => $domisili,'link' => $link,'format_nomor_surat' => $formatnomorsurat->id_no]);
	...
	public function permohonanCetak(Request $request,$id,$noFormat)
	{
		...
		$datasurat = Suratdomisili::where('nomor_surat',$id)->get();
		$desa = Desa::where('id_desa',$datasurat[0]->id_desa)->with('kabupaten', 'kecamatan')->get();

		$updateSurat = Suratdomisili::where('nomor_surat',$id);
