...
class suratimbController extends Controller
{
	...
	public function index($id_no)
	{
		...
		return view('formsurat/suratImb',['penduduk' => $penduduk,'id_noSurat' => $id_no]);
	}
	public function cetaksuratimb(Request $request,$id_noSurat)
	{
		...
		$datasurat = Suratimb::where('nomor_surat',$idinsertsurat)->get();

		// kalau sudah dipanggil semua data yang dibutuhkan tinggal di masukkan kedalam view suratnya
		$view = \View::make('surat/imb',['desa' =>$desa,'penduduk' => $penduduk, 'nosurat' =>$nosurat, 'tanggalsurat' => $tglSurat, 'datasurat' => $datasurat, 'kop' => $request['kop']]);

		$html_content = $view->render();


		PDF::SetTitle('Surat IMB PDF');
		PDF::AddPage();
		PDF::writeHTML($html_content, true, false, true, false, '');

		PDF::output('cetaksuratimb.pdf');
	}
	public function permohonan($namaSurat)
	{
		$imb = Suratimb::where('id_desa',auth()->user()->id_desa)->where('status_surat','belum diproses')->with('penduduk')->get();
		// sesuai dengan route
		$idsurat = 13;
		
		$formatnomorsurat = Nomorsurat::where('id_desa',auth()->user()->id_desa)->where('id_jenissurat','=',$idsurat)->first();
		$link = 'cetakPermohonanIMB';
		return view ('admindesa/permohonan_surat',['namaSurat' => $namaSurat, 'data' => $imb,'link' => $link,'format_nomor_surat' => $formatnomorsurat->id_no]);
