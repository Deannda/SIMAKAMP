...
class suratjalanController extends Controller
{
	...
	public function index($id_no)
	{
		...
		return view('formsurat/suratJalan',['penduduk' => $penduduk,'id_noSurat' => $id_no]);
	}
	public function cetaksuratjalan(Request $request,$id_noSurat)
	{
		...
		$insert = Suratjalan::create([
			'nik' => $request['nik'],
			'tanggal_surat' => $tglSurat,
			'desa' => $request['desakel'],
			'kecamatan' => $request['kecamatan'],
			'kabupaten' => $request['kabkot'],
			'provinsi' => $request['prov'],
			'tujuan' => $request['tujuan'],
			'jumlah_pengikut' => $request['jumpe'],
			'pengikut' => $request['ket_pengikut'],
		...
		$datasurat = Suratjalan::where('nomor_surat',$idinsertsurat)->get();

		// kalau sudah dipanggil semua data yang dibutuhkan tinggal di masukkan kedalam view suratnya
		$view = \View::make('surat/jalan',['desa' =>$desa,'penduduk' => $penduduk, 'nosurat' =>$nosurat, 'tanggalsurat' => $tglSurat, 'datasurat' => $datasurat, 'kop' => $request['kop']]);

		$html_content = $view->render();

		PDF::SetTitle('Surat Jalan PDF');
		PDF::AddPage();
		PDF::writeHTML($html_content, true, false, true, false, '');

		PDF::output('cetaksuratjalan.pdf');
	}

	public function permohonan($namaSurat)
	{
		$jalan = Suratjalan::where('id_desa',auth()->user()->id_desa)->where('status_surat','belum diproses')->with('penduduk')->get();
		// sesuai dengan route
		$idsurat = 14;
		
		$formatnomorsurat = Nomorsurat::where('id_desa',auth()->user()->id_desa)->where('id_jenissurat','=',$idsurat)->first();
		$link = 'cetakPermohonanJalan';
		return view ('admindesa/permohonan_surat',['namaSurat' => $namaSurat, 'data' => $jalan,'link' => $link,'format_nomor_surat' => $formatnomorsurat->id_no]);
