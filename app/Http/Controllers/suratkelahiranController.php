...
class suratkelahiranController extends Controller
{
	...
	public function index($id_no)
	{
		...
		return view('formsurat/suratKelahiran',['penduduk' => $penduduk,'id_noSurat' => $id_no]);
	}
	public function cetaksuratkelahiran(Request $request,$id_noSurat)
	{
		...
		$datasurat = Suratkelahiran::where('nomor_surat',$idinsertsurat)->with('penduduk','ayahh','ibuu')->get();
		// kalau sudah dipanggil semua data yang dibutuhkan tinggal di masukkan kedalam view suratnya
		$view = \View::make('surat/kelahiran',['desa' =>$desa,'penduduk' => $penduduk, 'nosurat' =>$nosurat, 'tanggalsurat' => $tglSurat, 'datasurat' => $datasurat, 'kop' => $request['kop']]);

		$html_content = $view->render();

		PDF::SetTitle('Surat Pernyataan Kelahiran PDF');
		PDF::AddPage();
		PDF::writeHTML($html_content, true, false, true, false, '');

		PDF::output('cetaksuratkelahiran.pdf');
	}
	public function permohonan($namaSurat)
	{
		$kelahiran = Suratkelahiran::where('id_desa',auth()->user()->id_desa)->where('status_surat','belum diproses')->with('penduduk')->get();
		// sesuai dengan route
		$link = 'cetakPermohonanKelahiran';
		return view ('admindesa/permohonan_surat',['namaSurat' => $namaSurat, 'data' => $kelahiran,'link' => $link]);
	...
	public function permohonanCetak(Request $request,$id)
	{
		...
		$formatnomorsurat = Nomorsurat::where('id_no','=',15)->first();
		...
		PDF::SetTitle('Surat Pernyataan Kelahiran');
		PDF::AddPage();
		PDF::writeHTML($html_content, true, false, true, false, '');
