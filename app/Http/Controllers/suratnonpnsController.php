...
class suratnonpnsController extends Controller
{
	...
	public function index($id_no)
	{
		...
		return view('formsurat/suratNonpns',['penduduk' => $penduduk,'id_noSurat' => $id_no]);
	}
	public function cetaksuratnonpns(Request $request,$id_noSurat)
	{
		...
		$datasurat = Suratnonpns::where('nomor_surat',$idinsertsurat)->get();

		// kalau sudah dipanggil semua data yang dibutuhkan tinggal di masukkan kedalam view suratnya
		$view = \View::make('surat/nonpns',['desa' =>$desa,'penduduk' => $penduduk, 'nosurat' =>$nosurat, 'tanggalsurat' => $tglSurat, 'datasurat' => $datasurat, 'kop' => $request['kop']]);

		$html_content = $view->render();

		PDF::SetTitle('Surat NonPNS PDF');
		PDF::AddPage();
		PDF::writeHTML($html_content, true, false, true, false, '');

		PDF::output('cetaksuratnonpns.pdf');
	}
	public function permohonan($namaSurat)
	{
		$nonpns = Suratnonpns::where('id_desa',auth()->user()->id_desa)->where('status_surat','belum diproses')->with('penduduk')->get();
		// sesuai dengan route
		$idsurat = 16;
		
		$formatnomorsurat = Nomorsurat::where('id_desa',auth()->user()->id_desa)->where('id_jenissurat','=',$idsurat)->first();
		$link = 'cetakPermohonanNonPNS';
		return view ('admindesa/permohonan_surat',['namaSurat' => $namaSurat, 'data' => $nonpns,'link' => $link,'format_nomor_surat' => $formatnomorsurat->id_no]);
	...
	public function permohonanCetak(Request $request,$id,$noFormat)
	{
		...
		$datasurat = Suratnonpns::where('nomor_surat',$id)->get();
		$desa = Desa::where('id_desa',$datasurat[0]->id_desa)->with('kabupaten', 'kecamatan')->get();

		$updateSurat = Suratnonpns::where('nomor_surat',$id);
		...
		PDF::SetTitle('Surat Non PNS PDF');
		PDF::AddPage();
		PDF::writeHTML($html_content, true, false, true, false, '');
